...
describe('App auth states', () => {
  ...
  it('shows loading, then the public shell; Sign in is reachable via Profile', async () => {
    ...
    await userEvent.click(await within(aside as HTMLElement).findByRole('button', { name: 'Profile' }))
    await userEvent.click(await screen.findByRole('button', { name: 'Sign in' }))
    expect(screen.getByRole('form', { name: /sign in/i })).toBeInTheDocument()
  }, 15000)

  it('shows the user email and Sign out on the Profile page when a session exists', async () => {
  ...
  it('signs out back to the anonymous shell', async () => {
    ...
    // Back on the anonymous Profile page: the Sign in affordance returns.
