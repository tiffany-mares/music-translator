...
- ~~Build the C++ DSP core only if Basic Pitch's output shows a measurable gap~~ — **explicitly skipped (Phase 6.5, 2026-08-01)** via the done-when's "confirmed unnecessary" branch: no feature in the shipped product consumes tempo/beat data (the player syncs on Whisper word timings; sing-along v1 uses live mic pitch; even `pitch.json` has no serving endpoint — 6.4's deferred prerequisite), so a benchmark would measure data with zero readers, and the only field evidence (Phase 1.4's listener verdict, `notes/phase1.md` §1.4) noted no rhythm/timing looseness. Revisit only if a rhythm-consuming feature (melody comparison, beat-snapped UI) lands together with the pitch-serving endpoint. See `notes/phase6.md` §6.5.
