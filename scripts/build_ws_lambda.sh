#!/usr/bin/env bash
# Phase 6.1: build both Go WebSocket Lambda zips via the golang:1.24 image
# (no host Go, per the no-host-toolchains convention; 1.24 = current AWS SDK
# v2 floor). Runs go mod tidy (keeps go.sum complete) and the unit tests
